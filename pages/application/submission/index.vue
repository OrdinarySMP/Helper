...
import { ApplicationSubmissionState, ApplicationSubmissionFilter } from "@/types/application/submission";
    ...
    >
      <template #search-bar>
        <FieldSelect
          ...
          @change="changeFilter"
        />
        <FieldInput name="discord_id" label="Discord ID" @change="changeFilter" />
      </template>
      <template #body-state="{ data }">
        {{ ApplicationSubmissionState[data.state] }}
      </template>
      <template #body-member="{ data }">
        {{ data.member?.nick ?? data.member?.user?.global_name ?? data.member?.user?.username ?? data.discord_id  }}
      </template>
