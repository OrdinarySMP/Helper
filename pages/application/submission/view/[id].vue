...
const updateSubmission = async (state: ApplicationSubmissionState, reason?: string) => {
  ...
  if (error.value) {
    successNotification(error.value.data.message, error.value.data.errors.join(", "))
  } else {
...
const applicationQuestionAnswers = computed(() => {
  const applicationQuestionAnswers = applicationSubmission.value?.application_question_answers ?? []
  return applicationQuestionAnswers.sort((a, b) => 
    a.application_question.order - 
    b.application_question.order
  );
})

const userName = computed(() => {
  return applicationSubmission.value?.member?.nick
...
<template>
  <div class="flex grow">
    ...
    <div v-else class="w-full">
      <p class="mb-8 text-2xl">Application Submission from {{ userName }} for {{ applicationSubmission?.application.name }}</p>
      <div class="grid grid-cols-1 gap-4">
        <div
          v-for="(answer, key) in applicationQuestionAnswers"
          :key="key"
        >
        ...
        <div
          v-if="applicationSubmission?.state === ApplicationSubmissionState.Pending"
          class="flex gap-2"
        >
          <Button
            class="mr-2 px-4"
    ...
    >
      ...
      <template #body>
        <p class="mb-2">
          Do you want to update this application to {{ ApplicationSubmissionState[toUpdateState] }}?
        </p>
        <FieldTextArea name="reason" label="Reason" />
      </template>
      <template #footer>
        <Button class="ml-4 px-4" size="sm" @click="updateSubmissionWithReason()">
          Update
        </Button>
        <Button
          class="px-4"
